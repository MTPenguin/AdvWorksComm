...
module.exports = (app, { getRouter }) => {
  ...
  ************************************************************************************************************************************/
  ...
  app.on('push', async (context) => {
    ...
    //   await $`${built}`
    // }
    const cmdLn = exec => async options => exec`flyway -community -user=${process.env.DB_USERNAME} -password=${process.env.DB_PASSWORD} -configFiles=../flyway.conf -locations=filesystem:../migrations ${options.cmds} -url=${process.env.DB_JDBC} -outputType=json`
